...
for(i in 1:length(dates)){
  ...
  the_url = paste0("https://api.coronavirus.data.gov.uk/v2/data?areaType=nation&areaCode=E92000001&metric=newCasesBySpecimenDate&format=csv&release=",as.character(date))
  print(date)
...
```{r}
library(ggrepel)
p<-ggplot(all%>% filter(date>"2020-12-02"),aes(y=newDeaths28DaysByDeathDate,color=as.factor(date),x=data_date-date,group=date,label=as.character(date)))+geom_line()+labs(y="Number of deaths reported",x="Days after date of death",color="Date of death") + geom_label_repel(label.size=NA, fill = alpha(c("white"),0.5),label.padding=0.2,min.segment.length = 0,size=2,data=all %>% group_by(date) %>% filter(data_date=="2021-1-1")%>% filter(date>"2020-12-02"),segment.color="black")+theme_bw()+
...
library(ggrepel)
ggplot(all%>% filter(date>"2020-12-06"),aes(y=newCasesBySpecimenDate,color=as.factor(date),x=data_date-date,group=date,label=as.character(date)))+geom_line()+labs(y="Number of cases",x="Days after specimen date",color="Specimen date") + geom_label_repel(label.size=NA, fill = alpha(c("white"),0.5),label.padding=0.2,min.segment.length = 0,size=2,data=all %>% group_by(date) %>% filter(data_date=="2021-1-5")%>% filter(date>"2020-12-06"),segment.color="black")+theme_bw()+
